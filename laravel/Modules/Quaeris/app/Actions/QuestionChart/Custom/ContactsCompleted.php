...
class ContactsCompleted
{
    ...
    public function execute(QuestionChart $questionChart, string $group_by, string $sort_by, ?AnswersFilterData $answersFilterData = null): AnswersChartData
    {
        ...
        // $tot_invited = array_sum(array_column(array_column($emails, 'value'), 'invited')) + array_sum(array_column(array_column($smss, 'value'), 'invited'));
        // $tot_risp = array_sum(array_column(array_column($emails, 'value'), 'answers')) + array_sum(array_column(array_column($smss, 'value'), 'answers'));
        ...
        $res = AnswersChartData::from(['answers' => $answers, 'chart' => $questionChart->charts[0]]);
