...
- **Cross-module awareness** – il nuovo setup di PHPStan del modulo Sigma aggiunge `scanDirectories` verso `Modules/Ptv`, `Modules/Progressioni` e `Modules/Performance` per eliminare falsi positivi sui trait condivisi. I temi che richiamano componenti o trait di quei moduli devono mantenere lo stesso approccio (aggiungere le rispettive directory a `scanDirectories`) per beneficiare degli stessi fix.
